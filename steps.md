...
- [x] **7.17 Team names in IN_GAME_PENALTY chips + scoring explanation popover** ✅
  - Done 2026-06-08. UI clarity + scoring education, layered on top of 7.16. Two user-facing changes: (1) the IN_GAME_PENALTY chips now display the team name (truncated to 7 chars) instead of "Home team" / "Away team"; (2) a clickable "!" icon next to the IN_GAME_PENALTY label opens a popover with 7 structured scoring examples. **No scoring logic changes** — the canonical HOME/AWAY values are unchanged, the save-batch validator is unchanged, the strategy is unchanged.
  - **Team-name lookup function.** `components/matches/MatchBettingForm.tsx` — new `teamLabel(code, homeTeam, awayTeam)` helper next to the existing label maps. For `code === "HOME"` it returns the first 7 chars of `match.homeTeam` (with `…` suffix if truncated); for `code === "AWAY"` it returns the truncated `match.awayTeam`. The full team name is exposed as `fullName` so the chip's `title` attribute can show the un-truncated name on hover. The static `IN_GAME_PENALTY_LABELS` map (HOME → "Home team", AWAY → "Away team") is retained as the **fallback** for the edge case where a match has no `homeTeam` / `awayTeam` (preserves ISC 6).
  - **InGamePenaltyRow updated.** Now accepts `homeTeam: string` and `awayTeam: string` props. The chip text uses `teamLabel(...).display` and the chip's `title` attribute uses `teamLabel(...).fullName` (only set when the team name was actually used — null when the static label fallback fired, so the tooltip is suppressed for the legacy edge case). Storage value is still `HOME` / `AWAY`; only the displayed text changes.
  - **Locked-state display also updated.** `LockedMarketRow` and `formatValue` are updated. The row receives `homeTeam` / `awayTeam` as props (passed through from the form for every market row — overkill for non-penalty markets but cheap and consistent). `formatValue` now takes optional `homeTeam` / `awayTeam` and the `IN_GAME_PENALTY` branch uses `teamLabel(...).display` instead of the static label map. The `EXACT_SCORE` and `HALF_SCORING` branches are unchanged.
  - **"!" icon.** Inline next to the "Which team gets an in-game penalty?" label in `InGamePenaltyRow` (editable form only — the locked-state rows don't need it). A 4×4 circle with a bold `!` glyph, `border` + `bg-background/40`, `text-muted-foreground` that brightens on hover. ARIA: `aria-label="How is the score calculated?"`, `aria-expanded={open}`.
  - **ScoringExplainedPopover — `components/matches/ScoringExplainedPopover.tsx`.** New client component. Renders the "!" trigger button + a popover panel. Click-to-open; click-outside OR Escape key OR explicit close (✕) button dismisses. The popover uses the existing design system: `paper-card` outer container, `micro-label` for the header ("How scoring works"), bordered `bg-background/40` blocks for each example. Anchored top-right of the trigger (`absolute right-0 top-6 z-50`), `w-[min(92vw,32rem)]` so it fits phone screens and doesn't grow too wide on desktop. Scrollable example list (`max-h-[70vh] overflow-y-auto`).
  - **Scoring examples data — `lib/services/scoring-examples.ts`.** New file. Exports `SCORING_EXAMPLES: ScoringExample[]` (7 entries) and the `ScoringExample` type. The type models the full shape the popover needs: `title`, `match`, `stage`, `userBet` (exactScore / halfScoring / inGamePenalty), `result` (finalScore / winner / halfScoring / inGamePenalty), `breakdown[]` (per-market row: market, pick, points, note), `total`, `explanation`. The 7 examples cover the 7 scenarios the user requested:
    1. **Tunisia vs Canada** (group, user's example) — +3 pts — half-scoring miss floors at -1.
    2. **Brazil vs Argentina** (group, perfect bet) — +9 pts — all three markets hit, max score.
    3. **Mexico vs South Africa** (group, all wrong) — -1 pt — the worst-case floor demonstration.
    4. **France vs Spain** (group, no penalties awarded) — +3 pts — penalty market voided because no penalties were awarded.
    5. **Germany vs Portugal** (group, both teams penalised) — +3 pts — penalty market skipped (manual settlement), floor still applies.
    6. **USA vs England** (knockout) — +12 pts — demonstrates the higher knockout weights (+5 exact, +2 winner).
    7. **Morocco vs Croatia** (group, big miss) — -2 pts — both half-scoring picks wrong → -2 clamped to -1, penalty miss → -1.
  - **Per-example block structure** (in the popover): bold title with "1." / "2." prefix + a `micro-label` for the stage (GROUP / KNOCKOUT); a 3-row `<dl>` (Match / Your bet / Result) in monospace; a `<ul>` of the breakdown rows, each prefixed by a coloured points badge (emerald for +ve, destructive for -ve, muted for 0); a footer line showing "Total: +N" + the explanation. The whole block is `border border-border bg-background/40 p-3`.
  - **Dismissal.** `useEffect` adds `pointerdown` (click-outside) and `keydown` (Escape) listeners only while the popover is open. The `useRef` points to a wrapper that contains both the trigger and the panel, so clicks on the panel itself don't close it. Cleanup runs on close/unmount.
  - **Layer scope.** Touched: `lib/services/scoring-examples.ts` (new), `components/matches/ScoringExplainedPopover.tsx` (new), `components/matches/MatchBettingForm.tsx` (teamLabel helper + InGamePenaltyRow props + LockedMarketRow props + formatValue signature), `steps.md`. NOT touched: `prisma/schema.prisma`, `prisma/migrations/*`, `lib/scoring/*` (no logic changes), `lib/services/ingest-*` (no data changes), `lib/services/save-bets-batch.ts` (no validation changes — the canonical codes HOME/AWAY are unchanged), the strategy files, the seed, the wipe scripts, the admin settlement UI.
  - **Verification.** `npx next build` ✓ · `ls lib/services/scoring-examples.ts` exists · `ls components/matches/ScoringExplainedPopover.tsx` exists · `grep "homeTeam" components/matches/MatchBettingForm.tsx` shows the team-name lookup in InGamePenaltyRow (4 hits: helper definition, prop type, prop destructure, prop usage) · `grep "ScoringExplainedPopover" components/matches/MatchBettingForm.tsx` shows the popover is imported and rendered (2 hits) · `grep "7.17" steps.md` shows the new phase · `grep "SCORING_EXAMPLES" lib/services/scoring-examples.ts` shows the 7 examples exported (definition + 1 export). The popover and the team-name chips are exercised manually by the user on a group's matches page.

- [x] **7.18 HALF_SCORING team names + scoring popover at top of matches page** ✅
  - Done 2026-06-08. Two small UI clarity fixes on top of 7.17. (1) The HALF_SCORING chips in `MatchBettingForm` now display the team name (truncated to 7 chars) followed by " 1H" / " 2H" instead of the static "Home 1H" / "Away 1H" / etc. labels; the locked-state display uses the same team-name labels. (2) The scoring-examples popover is hoisted to the top of the group's matches page — a single "How scoring works" button in the page header (not per-match) opens the same 7-example popover. The inline "!" trigger in `InGamePenaltyRow` is removed. **No scoring logic, schema, or example-data changes** — only display + placement.
  - **HALF_SCORING team-name helper — `components/matches/MatchBettingForm.tsx`.** New `halfScoringLabel(code, homeTeam, awayTeam)` helper next to the existing `teamLabel()`. Mapping: `A_*` codes use `match.homeTeam` + " 1H"/" 2H" suffix, `B_*` codes use `match.awayTeam` + the same suffix. The team-name truncation (7 chars, `…` suffix when longer) is reused from the existing `truncate()` helper — the half suffix is always appended to the *truncated* display name (e.g. "Argenti… 1H") and to the *full* `fullName` (e.g. "Argentina 1H"). Falls back to a `HALF_SCORING_FALLBACK` map (`A_1H` → "Home 1H", etc.) when the team name is missing, then to the raw code as a last resort. **Canonical storage values are unchanged** — the chip still toggles `A_1H` / `A_2H` / `B_1H` / `B_2H`; the save-batch validator and the scoring strategy are untouched.
  - **`HALF_SCORING_LABELS` map removed.** The static `{ A_1H: "Home 1H", … }` map (introduced in 7.9, last used in 7.10/7.10.1) is deleted from `MatchBettingForm.tsx`. The `HALF_SCORING_FALLBACK` map replaces it for the missing-team-name edge case only — its values are identical to the old `HALF_SCORING_LABELS` so the fallback behaviour is unchanged.
  - **`HalfScoringRow` accepts team-name props.** New `homeTeam: string` and `awayTeam: string` props on the sub-component. The chip text uses `halfScoringLabel(...).display` and the chip's `title` attribute uses `halfScoringLabel(...).fullName` (the full "Argentina 1H" / "Argentina 2H" string — same hover behaviour as `InGamePenaltyRow`). The form passes `match.homeTeam` / `match.awayTeam` down from `MatchBettingForm` (same pattern as `InGamePenaltyRow`).
  - **`formatValue` HALF_SCORING branch updated.** The locked-state display now uses `halfScoringLabel(code, homeTeam, awayTeam).display` for each picked code. `formatValue` already accepted optional `homeTeam` / `awayTeam` (added in 7.17) so the signature is unchanged. Saved values like `"A_1H,B_2H"` now render as e.g. `"Argentina 1H + Germany 2H"` in the locked-state row.
  - **`ScoringExplainedPopover` accepts a `label` prop.** `components/matches/ScoringExplainedPopover.tsx` — the function signature gains `{ label?: string } = {}` (default `"!"`). When `label === "!"` the trigger keeps the original 4×4 circular icon styling (back-compat for any future inline usage); when `label` is anything else (e.g. `"How scoring works"`), the trigger re-skins to a `command-strip` button sized like the other top-level CTAs in the design system (`px-3 py-1.5 text-xs font-bold`). The popover panel, the `useEffect` dismissal, the `SCORING_EXAMPLES` import, and the `ExampleBlock` component are all UNCHANGED.
  - **Inline "!" trigger removed from `InGamePenaltyRow`.** The `<div className="flex items-center gap-2"><p>…</p><ScoringExplainedPopover /></div>` wrapper is collapsed back to a single `<p>` for the label. The popover component is no longer imported in `MatchBettingForm.tsx`. One popover, one trigger, page-level.
  - **Top-level button — `app/(app)/groups/[groupId]/matches/page.tsx`.** The page imports `ScoringExplainedPopover` and renders `<ScoringExplainedPopover label="How scoring works" />` in its own `<div className="mb-8">` slot, placed between the "X members · predictions lock 5 minutes before kickoff." paragraph (whose `mb-8` is reduced to `mb-4` so the button has its own breathing room) and the `<MatchList>`. The page is a Server Component; the popover is a Client Component — Next.js handles the boundary. The "!" trigger on the inline render is gone; the user now clicks one button at the top of the page to see all 7 examples.
  - **Layer scope.** Touched: `components/matches/MatchBettingForm.tsx` (helper + HalfScoringRow props + formatValue branch + remove popover import + remove inline trigger), `components/matches/ScoringExplainedPopover.tsx` (label prop + conditional trigger styling), `app/(app)/groups/[groupId]/matches/page.tsx` (import + button placement + spacing tweak), `steps.md`. NOT touched: `prisma/schema.prisma`, `prisma/migrations/*`, `lib/scoring/*` (no logic changes), `lib/services/scoring-examples.ts` (the 7 examples data is unchanged), `lib/services/save-bets-batch.ts` (validation unchanged — canonical codes preserved), `lib/services/ingest-*` (no data shape changes), the seed JSON, the wipe scripts, the admin settlement UI, the Settlement Hub, the `MatchList` / `MatchCard` components.
  - **Verification.** `npx next build` ✓ · `grep "HALF_SCORING_LABELS" components/matches/MatchBettingForm.tsx` returns ZERO matches (the old map is deleted) · `grep "homeTeam\|awayTeam" components/matches/MatchBettingForm.tsx` shows team-name lookups in both `HalfScoringRow` and the HALF_SCORING branch of `formatValue` · `grep "ScoringExplainedPopover" components/matches/MatchBettingForm.tsx` returns ZERO matches (the import and inline trigger are gone) · `grep "ScoringExplainedPopover" "app/(app)/groups/[groupId]/matches/page.tsx"` shows the import + the new top-level button (2 hits) · `grep "label" components/matches/ScoringExplainedPopover.tsx` shows the new prop (3 hits: destructure, type annotation, usage) · `grep "7.18" steps.md` shows the new phase. The team-name chips and the top-level popover are exercised manually by the user.

- [x] **7.19 Yellow command buttons + centered scrollable scoring popover** ✅
  - Done 2026-06-08. Visual + UX polish on top of 7.18: a new `command-strip-flat` utility class gives both yellow call-to-action buttons (Save predictions, How scoring works) a flat solid yellow surface (no gradient), and the scoring-examples popover is converted to a centered modal with a backdrop, an Escape close, and a scrollable card. **No scoring logic, schema, or example-data changes** — only display + popover positioning.
  - **New utility class — `command-strip-flat` in `app/globals.css`.** Added right after the `command-strip:focus-visible` block. Solid yellow background (`oklch(0.88 0.18 95)` — hue 95 = yellow, not orange), dark text, fully-rounded pill shape, `font-weight: 700`, soft yellow drop-shadow. `:hover` brightens (filter brightness 1.08) + nudges 1px up; `:focus-visible` adds the standard ring; `:disabled` applies the usual opacity + pointer-events-none. **The `command-strip` class is UNCHANGED** — other buttons that rely on the gradient (e.g. the marketing landing CTA in `app/page.tsx`, the admin "+ Onboard league" button, the Sync now button) keep their existing look. `command-strip` and `command-strip-flat` are siblings, not overrides.
  - **Save predictions button — `components/matches/MatchBettingForm.tsx`.** Class swap: `"command-strip px-5 py-2 text-sm font-bold disabled:opacity-50 disabled:pointer-events-none"` → `"command-strip-flat px-5 py-2 text-sm disabled:opacity-50 disabled:pointer-events-none"`. `font-bold` dropped (the new class sets `font-weight: 700`); `disabled:opacity-50 disabled:pointer-events-none` kept inline (the new class already disables `pointer-events` on `:disabled` but the inline opacity gives a slightly stronger visual de-emphasis, matching the previous behaviour). Sizing (`px-5 py-2 text-sm`) stays inline so the two yellow buttons (Save + How scoring works) can have different padding without proliferating class variants.
  - **How scoring works button — `components/matches/ScoringExplainedPopover.tsx`.** Same class swap on the top-level trigger: `"command-strip px-3 py-1.5 text-xs font-bold"` → `"command-strip-flat text-xs"`. The new class's default `padding: 0.5rem 1.25rem` (≈ `py-2 px-5`) replaces the inline `px-3 py-1.5`. The `!` trigger (the 4×4 inline glyph, default label) is UNCHANGED — only the page-level button uses the new class.
  - **Popover → centered modal.** The popover transitions from an anchored panel (`absolute right-0 top-6 z-50 w-[min(92vw,32rem)]`) to a full-viewport overlay (`fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm`). The card sits inside, centered both ways by the flex container, with `paper-card p-6 max-w-2xl w-full max-h-[85vh] overflow-y-auto`. The card has `onClick={(e) => e.stopPropagation()}` so clicks inside don't bubble to the backdrop's close handler.
  - **Dismissal paths (all three required by ISC 9-11).**
    1. **Backdrop click** — the outer `div` runs `onClick={() => setOpen(false)}`.
    2. **✕ button** — the existing close button in the card header still calls `setOpen(false)`.
    3. **Escape key** — a `useEffect` registers a `keydown` listener for the duration the popover is open; cleanup runs on close/unmount. The previous click-outside-via-`useRef` mechanism is removed (the backdrop click is the new outside-click path; the `useRef` is no longer needed and was deleted).
  - **Trigger behaviour change.** The trigger previously toggled (`onClick={() => setOpen((v) => !v)}`); it now always opens (`onClick={() => setOpen(true)}`). Closing is exclusively the three paths above. This is intentional — with a centered modal, re-clicking the trigger is rarely the user's intent (they usually want the modal gone).
  - **Z-index.** The overlay uses `z-50`, matching the previous absolute popover's z-index. The card itself does not need a higher z-index — it's a child of the flex-centered overlay, which is the only top-layer element. Other UI (header, page content) is z-50 or below in the design system; nothing else uses a higher z-index, so the popover is always on top.
  - **Scrollability.** The card's `max-h-[85vh] overflow-y-auto` ensures the 7 examples fit on small screens — the inner example list is no longer wrapped in a separate `max-h-[70vh] overflow-y-auto` (the card itself now scrolls). The `pr-1` on the inner list is kept as a small gutter against the scrollbar. **All 7 examples are preserved verbatim** — the data, the `ExampleBlock` rendering, the `SCORING_EXAMPLES` import, and the example-row markup are all UNCHANGED.
  - **Layer scope.** Touched: `app/globals.css` (new class), `components/matches/MatchBettingForm.tsx` (button class swap), `components/matches/ScoringExplainedPopover.tsx` (button class swap + popover positioning rewrite + `useRef` removal), `steps.md`. NOT touched: `lib/services/scoring-examples.ts` (the 7 examples are unchanged), `lib/scoring/*` (no logic changes), `app/(app)/groups/[groupId]/matches/page.tsx` (the page-level trigger usage is unchanged — only the button styling inside `ScoringExplainedPopover` changed), `app/page.tsx` (the marketing CTA still uses `command-strip`), the admin pages (their buttons still use `command-strip`), the `MatchList` / `MatchCard` components, the seed, the wipe scripts, the schema.
  - **Verification.** `npx next build` ✓ · `grep "command-strip-flat" app/globals.css` shows the new class · `grep "command-strip-flat" components/matches/MatchBettingForm.tsx` shows the Save predictions button uses the new class · `grep "command-strip-flat" components/matches/ScoringExplainedPopover.tsx` shows the top-level trigger uses the new class · `grep "fixed inset-0" components/matches/ScoringExplainedPopover.tsx` shows the centered overlay · `grep "max-h-\[" components/matches/ScoringExplainedPopover.tsx` shows the scrollable card · `grep "command-strip " components/matches/MatchBettingForm.tsx` returns ZERO (the old class is removed from the Save predictions button) · `grep "7.19" steps.md` shows the new phase. The two yellow buttons and the centered popover are exercised manually by the user on a group's matches page.
